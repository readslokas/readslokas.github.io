      ...
      if (expandedIndex === null) {
        ...
        finerMap[speedTable[curr]].forEach(({ label, value }) => {
          ...
          fineBtn.onclick = () => {
            ...
            // when a finer button is clicked, show only its branch
    ...
    function insertNextFiner(base, label, value, nextIndex) {
      ...
      // Reset finerMap for this base every time a finer is chosen
      finerMap[base] = [
        { label, value } // keep only the current clicked one
      ];

      // Insert the next finer option if valid
      if (num + step < speedTable[nextIndex]) {
        finerMap[base].push({
          label: `${nextFine}x`,
          value: value + 5
        });
